...
// returns the radius of an ellipse for a given angle
double Ellipse::radius(const int angle) const
{
        return (major() * minor()) / (sqrt(pow(minor(), 2)*pow(sin(angle* M_PI / 180), 2) + pow(major(), 2)*pow(cos(angle* M_PI / 180), 2)));
}
...
void Arc::draw_lines() const
{
    if (color().visibility())
    {
        fl_arc(point(0).x , point(0).y, w, h, d_start, d_stop); 
    }
}
...
void Box::draw_lines() const
{
    if (color().visibility())
    {
        /**  decided against this implementation because i would have to add complexity for drawing shorter lines
        for (int i = 0 ; i < number_of_points() ; i++)
        {
            fl_line(point(i).x, point(i).y, point((i + 1) % 4).x, point((i + 1) % 4).y);
        }
        **/
...
void Arrow::draw_lines() const
{
    ...
    double x_diff = point(0).x - point(1).y;
    double y_diff = point(0).y - point(1).y;

    double slope_angle = atan2(y_diff, x_diff) * 180 / M_PI;

    fl_line(point(0).x, point(0).y, point(0).x - cos((slope_angle + 45) * M_PI / 180) * 20, point(0).y - sin((slope_angle + 45) * M_PI / 180) * 20);
    fl_line(point(0).x, point(0).y, point(0).x - cos((slope_angle - 45) * M_PI / 180) * 20, point(0).y - sin((slope_angle - 45) * M_PI / 180) * 20);
}
