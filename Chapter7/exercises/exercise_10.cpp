...
const string intro = "Welcome to 'the calculator' type help for instructions.";
...
const string help_message6 = "Finally to print your value enter ';' and to quit this program enter 'quit'";
...
Token Token_stream::get()
{
	...
	default:
		if (isalpha(ch)) {
			...
			if (s == "help") return Token(show_instructions);
			return Token(name, s);														
		}
		error("Bad token");
	}
...
void Token_stream::ignore(char c)
{
	...
	while (cin.get(ch))
	{
		cin.putback(ch);													// if print is not found it will ignore all remaining input and return
		Token t = ts.get();	
		if (t.kind ==c) return;
	}
	return;
...
double primary()
{
	...
	case square_root:
	{
		double expression = narrow_cast<int>(primary());
		if (expression < 0)
		{
	...
	{																// function executed when their tokens are caught in the case. If i were 
		...
		double x = narrow_cast<int>(primary());
		t = ts.get();
		if (t.kind != ',')
		{
			error("',' expected");
		}
		int i = narrow_cast<int>(primary());											// the book asks to require i to be an integer using the technique we used for 
...
double term()
{
	double left = narrow_cast<int>(primary());
	while(true) {
		Token t = ts.get();
		switch(t.kind) {
		case '*':
			...
			break;
		case '/':
		{	double d = narrow_cast<int>(primary());
